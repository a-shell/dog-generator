...
fetch(BREEDS_URL)
    ...
    select.addEventListener("change", event => {
        ...
        doggoInfo.assignMF(); //Assigns a gender and name from the m/fNames lists
        doggoInfo.assignAge(); //Calls assignAge method which assigns an age
    ...
    const doggoInfo = {
        ...
        assignMF() { //This will assign a gender to the dog and a name from the male or female lists
            x = (Math.floor(Math.random() * 2) == 0); //x will be either 0 or 1 and will evaluate to true if 0 and false if 1
            if(x) { //Truthy statement where if x = true, sex will be female and the assignNames method will run with its parameter being the fsNames array
                this.MF = "Female";
                this.assignName(this.fNames);
            } else { //Truthy statement where if x = false, sex will be male and the assignNames method will run with its parameter being the mNames array
                this.MF = "Male";
        ...
        assignLikes() {
            this.likes = this.yatesShuffle(this.likesList).slice(0,2); //Randomizes the likesList array using the yatesShuffle method and selects the first two elements
            document.getElementById("likes").innerHTML = `Likes: ${this.likes[0]}, ${this.likes[1]}`; //Places the selected elements into the string
        },

        assignDislikes() {
            this.dislikes = this.yatesShuffle(this.dislikesList).slice(0,2); //Same as assignLikes
